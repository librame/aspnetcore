...
namespace Librame.AspNetCore.Gallery
{
    ...
    /// 图库图片。
    /// </summary>
    public class GalleryPicture : GalleryPicture<string, int, string, int, DateTimeOffset>
    {
        /// <summary>
        /// 构造一个 <see cref="GalleryPicture"/> 实例。
        /// </summary>
        public GalleryPicture()
            : base()
        {
            // 默认使用空标识符，新增推荐使用服务注入
            EditorId = Id = GuIdentifier.Empty;
        }
    }
    ...
    public class GalleryPicture<TId, TCategoryId, TEditorId, TSourceId, TDateTime> : AbstractEntity<TId>, IPublishing<TDateTime>
