...
namespace Librame.AspNetCore.UI
{
    ...
    class ResetDefaultApplicationModelProvider : IApplicationModelProvider
    {
        ...
        public ResetDefaultApplicationModelProvider(
        ...
        internal ControllerModel CreateControllerModel(TypeInfo typeInfo)
        {
            ...
            if (controllerModel.ControllerType.TryGetCustomAttribute(out GenericApplicationModelAttribute _))
                controllerModel.ControllerName = typeInfo.GetBodyName();
            else
                controllerModel.ControllerName = typeInfo.Name;

            if (controllerModel.ControllerName.EndsWith("Controller", StringComparison.OrdinalIgnoreCase))
                controllerModel.ControllerName = controllerModel.ControllerName.Substring(0, controllerModel.ControllerName.Length - "Controller".Length);
