...
namespace Librame.AspNetCore.Web.Projects
{
    ...
    internal class ResetApplicationModelFactory
    {
        ...
        public ResetApplicationModelFactory(
            ...
            List<string> errors)
        {
            ...
            catch (InvalidOperationException ex)
            {
                ...
                //    Environment.NewLine,
                //    ex.Message);
                var message = $"For action: '{action.DisplayName}'{Environment.NewLine}Error: {ex.Message}";
                errors.Add(message);
            }
        }
        ...
        private static List<string> AddErrorNumbers(IEnumerable<string> namedRoutedErrors)
        {
            ...
            return namedRoutedErrors
                .Select((error, i) =>
                    $"Error {i + 1}:{Environment.NewLine}{error}")
                .ToList();
        }
                ...
                for (var i = 1; i < actions.Count; i++)
                {
                    ...
                    if (!template.Equals(other, StringComparison.OrdinalIgnoreCase))
                    {
                        var descriptions = actions.Select(a =>
                        {
                            ...
                            return $"Action: '{a.action.DisplayName}' - Template: '{a.selector.AttributeRouteModel.Template}'";
                        });

                        // Resources.FormatAttributeRoute_DuplicateNames(routeName, Environment.NewLine, string.Join(Environment.NewLine, descriptions));
                        var message = $"Attribute routes with the same name '{routeName}' must have the same template:{Environment.NewLine}{string.Join(Environment.NewLine, descriptions)}";
            ...
            List<(ActionModel action, SelectorModel selector)> actions)
        {
            ...
            // Resources.AttributeRoute_NullTemplateRepresentation;
            var nullTemplate = "(none)";
            ...
            for (var i = 0; i < actions.Count; i++)
            {
                ...
                //    routeTemplate,
                //    formattedVerbs);
                var description = $"Action: '{action.DisplayName}' - Route Template: '{routeTemplate}' - HTTP Verbs: '{formattedVerbs}'";
            ...
            return $"A method '{formattedMethodInfo}' must not define attribute routed actions and non attribute routed actions at the same time:{Environment.NewLine}{string.Join(Environment.NewLine, actionDescriptions)}{Environment.NewLine}{Environment.NewLine}Use 'AcceptVerbsAttribute' to create a single route that allows multiple HTTP verbs and defines a route, or set a route template in all attributes that constrain HTTP verbs.";
        }
        ...
        private static string CreateAttributeRoutingAggregateErrorMessage(IEnumerable<string> individualErrors)
        {
            ...
            var message = $"The following errors occurred with attribute routing information:{Environment.NewLine}{Environment.NewLine}{string.Join(Environment.NewLine + Environment.NewLine, errorMessages)}";
            return message;
        }
    }
}
