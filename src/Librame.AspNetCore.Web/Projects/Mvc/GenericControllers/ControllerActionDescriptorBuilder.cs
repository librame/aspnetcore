...
namespace Librame.AspNetCore.Web.Projects
{
    ...
    internal static class ControllerActionDescriptorBuilder
    {
        ...
        {
            return ResetApplicationModelFactory.Flatten(application, CreateActionDescriptor);
        }
            ...
            ActionModel action)
        {
            ...
            if (isVisibleSetOnActionOrController && !IsAttributeRouted(actionDescriptor))
            {
                ...
                throw new InvalidOperationException($"The action '{actionDescriptor.DisplayName}' has ApiExplorer enabled, but is using conventional routing. Only actions which use attribute routing support ApiExplorer.");
            }
